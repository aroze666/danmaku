...
public:
  ...
  void Update(sf::Vector2f position, sf::Vector2f aim_direction, Scene &scene) {
    ...
    for (int i = 0; i < rays.size(); i++) {
      rays[i].angle = angle;
      sf::Vector2f d = {
          (float)std::sqrt(1 + std::pow(direction.y / direction.x, 2)),
          (float)std::sqrt(1 + std::pow(direction.x / direction.y, 2))};
      ...
      if (direction.x < 0)
        rays[i].setRotation(180 +
                            std::atan(direction.y / direction.x) * 180 / PI);
      else
...
private:
  virtual void draw(sf::RenderTarget &target, sf::RenderStates states) const {
    for (int i = 0; i < rays.size(); i++) {
      ...
      float line_height = 64 * 512 / rays[i].getSize().x /
                          std::cos(player_angle - rays[i].angle);
