...
void Player::SetHitboxSize(sf::Vector2f size) { this->hitbox.setSize(size); }
void Player::SetBulletSize(sf::Vector2f size) { this->bullet_size = size; }
void Player::SetPosition(sf::Vector2f position) {
  this->hitbox.setPosition(position);
}
void Player::SetVelocity(sf::Vector2f velocity) { this->velocity = velocity; }

void Player::SetMovementRange(sf::Vector2f range) {
  this->movement_range = range;
}
void Player::SetWindow(sf::RenderWindow *window) { this->window = window; }
sf::Vector2f Player::GetPlayerCenter() {
  return {hitbox.getPosition().x + (hitbox.getSize().x / 2),
...
sf::Vector2f Player::GetPlayerAimVector() {
  ...
  aim_direction =
      aim_direction / (float)std::sqrt(std::pow(aim_direction.x, 2) +
                                       std::pow(aim_direction.y, 2));
  return aim_direction;
}
sf::RectangleShape Player::GetHitbox() { return hitbox; }
...
void Player::Event() {
  ...
  if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
    ...
    if (time > firing_cooldown) {
      ...
      float aim_offset = 0;
      if (rand() % 100 > 50)
        aim_offset = speed_factor * 0.05 *
                     (std::abs(this->velocity.x) + std::abs(this->velocity.y));
      else
        aim_offset = speed_factor * -0.05 *
                     (std::abs(this->velocity.x) + std::abs(this->velocity.y));
      if (moving) {
        ...
        if (aim_direction.x < 0 && aim_direction.y < 0)
          new_bullet.SetVelocity(
              {aim_direction.x + aim_offset, aim_direction.y - aim_offset});
  ...
  if (moving) {
    this->hitbox.setPosition(this->hitbox.getPosition() +
                             this->velocity * speed_factor);
  }
  ...
  if (hitbox.getPosition().x >= movement_range.x - hitbox.getSize().x)
    SetPosition({movement_range.x - this->hitbox.getSize().x,
  ...
  if (hitbox.getPosition().y >= movement_range.y - hitbox.getSize().y)
    SetPosition({this->hitbox.getPosition().x,
                 movement_range.y - this->hitbox.getSize().y});
}

bool Player::isbullethit(Bullet &bullet) {
  return bullet.GetHitStatus() || bullet.getPosition().x < 0 ||
         bullet.getPosition().x > movement_range.x ||
         bullet.getPosition().y < 0 ||
         bullet.getPosition().y > movement_range.y;
}

void Player::Update() {
  sf::Vector2f mouse_pos = {
      sf::Mouse::getPosition(*window).x - crosshair.getRadius(),
      sf::Mouse::getPosition(*window).y - crosshair.getRadius()};
