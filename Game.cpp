...
void Game::CollisionEvent() {
  for (auto enemy = enemy_list.begin(); enemy != enemy_list.end(); enemy++) {
    for (auto enemy_2 = std::next(enemy, 1); enemy_2 != enemy_list.end();
         enemy_2++)
      if (!enemy->IsHit() && !enemy_2->IsHit() &&
          is_Collide(enemy->GetHitbox(), enemy_2->GetHitbox())) {
    ...
    if (!enemy->IsHit() &&
        (is_Collide(player.GetHitbox(), enemy->GetHitbox()))) {
      enemy->SetHit(true);
  ...
  enemy_list.remove_if(
      [](Enemy enemy) { return enemy.IsHit() && !enemy.BulletExist(); });
}
