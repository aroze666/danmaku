...
void Enemy::SetVelocity(sf::Vector2f velocity) { this->velocity = velocity; }

void Enemy::SetHit(bool is_hit) { this->is_hit = is_hit; }
sf::RectangleShape Enemy::GetHitbox() { return hitbox; }
sf::Vector2f Enemy::GetVelcoity() { return velocity; }
...
void Enemy::FireBullet(sf::Vector2f position) {
  ...
  if (time > firing_cooldown && !is_hit) {
    ...
    sf::Vector2f center = {hitbox.getPosition().x + (hitbox.getSize().x / 2) -
                               new_bullet.getSize().x / 2,
                           hitbox.getPosition().y + (hitbox.getSize().y / 2) -
                               new_bullet.getSize().y / 2};
    ...
    if (rand() % 100 > 50)
      aim_offset =
          0.05 * (std::abs(this->velocity.x) + std::abs(this->velocity.y));
    else
      aim_offset =
          -0.05 * (std::abs(this->velocity.x) + std::abs(this->velocity.y));
